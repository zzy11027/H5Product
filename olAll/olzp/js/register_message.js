...
function send_capicha_regist() {
    ...
    if($('#getYzm').html()==='重新发送' || $('#getYzm').html()==='获取验证码'){
        $.ajax({
                url: 'http://al.douyar.cc/ol/express.php?step=send_sms',
                data: {
                    phone: mobile_phone
                    // captcha: yzm_captcha,
                },
...
//第二个验证码
function send_capicha_regists() {
   
    var mobile_phone = $('.inp2s').val();
    // var yzm_captcha=$('#captcha').val();
    // //alert(yzm_captcha);
    // var status=false;
    // if (mobile_phone == '') {
    ...
    if($('#getYzm').html()==='重新发送' || $('#getYzm').html()==='获取验证码'){
        $.ajax({
                url: 'http://www.aumantruck.com/mobile/om_zdp.php?step=send_sms',
                data: {
                    phone: mobile_phone
                    // captcha: yzm_captcha,
                },
